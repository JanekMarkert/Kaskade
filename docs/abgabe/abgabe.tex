...
ist der Anwendungsfall, der die Ortungskaskade motiviert
(Abschnitt~\ref{sec:leitmotiv}).
...
Die Transportmodus-Klassifikation (Abschnitte~\ref{sec:m1}
bis~\ref{sec:m4}, \ref{sec:messstudien}) ist der Anwendungsfall, der diese
Kaskade braucht; die Kaskade selbst, nicht die Klassifikation drumherum, ist
die Kernaussage der Arbeit und der Grund für den Namen. Die Klassifikation
trägt dazu ihre eigene Kernthese bei, in zwei Teilen: Apples
Systemklassifikator kann \emph{train} strukturell nicht erkennen, weil
\texttt{CMMotionActivity} den Zustand nicht kennt; ob das eigene Modell die
Klasse deshalb auch im Feld zuverlässiger trifft, ist die empirische Frage,
die M2 (Abschnitt~\ref{sec:m2}) beantwortet.
...
drei Referenzpunkte, berechnet mit
...
p2 & offener Himmel & 815 & 2{,}60 & 8{,}23 & 2{,}11 & 3{,}91 \\
...
seine eigene Unsicherheit realistisch ein. Drei Nebenbefunde bei der
Auswertung, alle desselben Musters: Die ursprünglich für p1, p2 und p3
eingetragenen Referenzkoordinaten wichen 155, 17 beziehungsweise 59~m vom
tatsächlichen Messpunkt ab, während die eigenen GNSS-Fixes jeweils über
mindestens zwei unabhängige Sessions auf denselben Fleck konvergierten
(unter 5~m Abstand zueinander bei p1, unter 5~m bei p2, unter 1~m bei
p3). Ursache war jedes Mal ein zu grob gesetzter Kartenpunkt bei der
Anlage der Referenzdaten, keine GNSS- oder Standortabweichung; alle drei
Koordinaten in \texttt{data/reference/reference\_points.geojson} wurden
entsprechend korrigiert. Bei p2 kamen zwei weitere, am 22.09. aufgezeichnete
Sessions (221 und 568~Fixes) zur ursprünglichen 26-Fix-Session hinzu; ihre
Median-Zentren liegen 1{,}3 und 3{,}2~m vom korrigierten Punkt entfernt, die
ursprüngliche Session dagegen durchweg rund 18~m davon -- ein eigener,
enger Block, der in der Tabelle als Teil des Fehlers stehen bleibt, weil
kein Protokollfehler dafür belegt ist, ähnlich den Ausreißern bei p1.
Dass die Referenzpunkt-Abweichung nun an allen drei Punkten auftrat, spricht
dafür, dass sie bei der Anlage systematisch war (grobes Antippen auf einer
kleinen Karte), nicht Zufall.
...
Gebäudefassaden stark aus. Genau dieses bimodale Verhalten wird in der
Literatur für Straßenschluchten erwartet und ist hier erstmals selbst
gemessen statt nur zitiert. p3 (abgeschattet) zeigt den Gegenpol: CEP50
und CEP95 liegen mit Faktor~4 nah beieinander, durchweg kleiner Fehler.
p2 (offener Himmel) liegt dazwischen, mit einem schwächeren Abbild
desselben p1-Musters -- 95~Prozent der 815~Fixes unter 8{,}4~m, aber ein
Ausreißer-Anteil von 3{,}8~Prozent bis 22~m, der überwiegend aus der 18~m
abseits liegenden ursprünglichen Session stammt.
